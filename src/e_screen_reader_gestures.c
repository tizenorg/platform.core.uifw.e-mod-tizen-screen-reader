...
_mouse_move(int type, Ecore_Event_Mouse_Button *event)
{
   ...
   if (event->multi.radius >= MAGIC_NUMBER || cover->flick_gesture.flick_to_scroll || cover->tap_n_hold_gesture_data.drag_start)
     {
        if (event->multi.radius >= MAGIC_NUMBER) event->multi.radius -= MAGIC_NUMBER;
